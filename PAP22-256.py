...
from measure import plt,np,npfarray,sqrt,pltext,curve_fit,val
...
pltext.initplot(num=1,title=r'$\sqrt{E_\alpha}$'+' als Funktion von Z',xlabel='Z',ylabel=r'$\sqrt{E_\alpha}/\sqrt{keV}$')
...
popt,pcov = curve_fit(fit_func_alpha,Z,sqrt_K_alpha,sigma=Delta_sqrt_K_alpha)
...
print(val('sig12_alpha',sig12_alpha,Delta_sig12_alpha))

# K_beta

### Rechnung nochmal ohne Ti weil der Wert Quatsch ist
...
pltext.initplot(num=2,title=r'$\sqrt{E_\beta}$'+' als Funktion von Z',xlabel='Z',ylabel=r'$\sqrt{E_\beta}/\sqrt{keV}$')
...
popt,pcov = curve_fit(fit_func_beta,Z,sqrt_K_beta,sigma=Delta_sqrt_K_beta)
...
print(val('sig12_beta',sig12_beta,Delta_sig12_beta))
