...
# Rüchardt
r_air = rh_2r_air / 2.
r_air_dsys = rh_2r_air / 2.
T_air = rh_50T_air / 50.
T_air_dsys = rh_50T_dsys / 50.
rh_k_air = 4. * rh_m_air * rh_V_air / (r_air**4 * T_air**2 * rh_p)
rh_k_air_dsys = 4. / (r_air**4 * T_air**2 * rh_p) * sqrt((rh_m_dsys * rh_V_air)**2 + (rh_m_air * rh_V_dsys)**2 + (rh_m_air * rh_V_air / (r_air**4 * T_air**2 * rh_p))**2 * ((4. * r_air**3 * r_air_dsys * T_air**2 * rh_p)**2 + (r_air**4 * 2. * T_air * T_air_dsys * rh_p)**2 + (r_air**4 * T_air**2 * rh_p_dsys)**2))

r_arg = rh_2r_arg / 2.
r_arg_dsys = rh_2r_arg / 2.
T_arg = rh_50T_arg / 50.
T_arg_dsys = rh_50T_dsys / 50.
rh_k_arg = (4. * rh_m_arg * rh_V_arg) / (r_arg**4 * T_arg**2 * rh_p)
rh_k_arg_dsys = 4. / (r_arg**4 * T_arg**2 * rh_p) * sqrt((rh_m_dsys * rh_V_arg)**2 + (rh_m_arg * rh_V_dsys)**2 + (rh_m_arg * rh_V_arg / (r_arg**4 * T_arg**2 * rh_p))**2 * ((4. * r_arg**3 * r_arg_dsys * T_arg**2 * rh_p)**2 + (r_arg**4 * 2. * T_arg * T_arg_dsys * rh_p)**2 + (r_arg**4 * T_arg**2 * rh_p_dsys)**2))
...
a1 = rh_m_dsys * rh_V_air
a2 = rh_m_air * rh_V_dsys
b = rh_m_air * rh_V_air / (r_air**4 * T_air**2 * rh_p)
a3 = b * 4. * r_air**3 * r_air_dsys * T_air**2 * rh_p
a4 = b * r_air**4 * 2. * T_air * T_air_dsys * rh_p
a5 = b * r_air**4 * T_air**2 * rh_p_dsys
c = 4. / (r_air**4 * T_air**2 * rh_p)
### much to high error of rh_k because the value is perfect
